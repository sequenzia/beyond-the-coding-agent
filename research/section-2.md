...
Current teaching definition: AgentOps keeps deployed agents observable, controlled, and accountable as the system changes. AgentOps names this talk's existing operational responsibilities; it introduces no new product, standard, or measured claim.
...
### AgentOps foundations, decisions, and pitfalls refinement, September 16, 2026

The presenter approved the copy and scripts for slides 37 through 39 at 0:55, 1:00, and 0:45. Their combined 2:40 preserves AgentOps at 4:10 and Section 2 at 28:30. This supersedes the earlier wording for these three slides. Slides 36 and 40 remain unchanged.

Foundations connects traces, configuration versions, and outcome metrics. The generic recorded trace contains retrieval, a model operation, a tool operation, and a check, with the model operation identified as one span. It represents observable activity, not private reasoning or measured telemetry. Keep sampled or delayed quality judgments and the distinction between healthy APIs and task success explicit. SLOs and error budgets remain backup.

Decisions covers useful protected evidence, evaluation before limited initial release exposure, comparison against a tested configuration, pause and rollback conditions, and accountable response. Permissions, permitted destinations, and resource limits are enforced outside the model. A configuration rollback does not undo completed actions. This develops the OpenTelemetry, Google SRE, and OWASP evidence already recorded in §3 and §6.

The presenter confirmed that CUI/ECI is familiar audience context and requested those abbreviations on slide 39 without expanding ECI. This uses the existing NARA and NIST evidence in §1, independent of the illustrative application. Review sensitive information across prompts, tool calls, and traces, with approved processing, access, and destinations enforced outside the model. CUI and ECI may overlap; neither the slide nor this teaching guidance establishes a particular service's approval or a compliance determination. The capability combination remains Willison's specific threat model, with the exact existing headline pitfall and probabilistic-filter limitation. The three slide specs carry no direct application references.
...
- Slides 37 through 39: the presenter approved the revised copy and scripts at 0:55, 1:00, and 0:45. Rehearse their combined 2:40, especially slide 38's release explanation and slide 39's CUI/ECI opening. Preserve the trace/span distinction, delayed quality judgments, protected evidence, controlled releases, enforced authority, accountable response, rollback limits, and exact shared headline pitfall. Editorial approval is complete; delivery and native playback remain rehearsal checks. Slides 36 and 40 are unchanged.
