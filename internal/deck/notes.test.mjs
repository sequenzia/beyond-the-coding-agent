...
[0:20] **[your story #2]** Keep the story slot.
...
test('all narrative slides end at their advance or Q&A handoff and preserve build cues', async () => {
  ...
  for (const section of [1, 2, 3]) {
    ...
    for (const file of (await fs.readdir(directory)).filter(name => name.endsWith('.md'))) {
      ...
      if (file.startsWith('24-')) assert.match(plain, /Autonomy is earned by evals, one step at a time\. Advance to slide 25 on the last word\.$/);
      if (file.startsWith('26-')) assert.doesNotMatch(plain, /Backup for the fifteen minutes|Do I need to learn/);
      count++;
    }
  }
