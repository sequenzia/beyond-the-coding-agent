...
**Mini-map.** A PNG at x 752, y 36, 160 by 90, from `internal/renders/`. The current area's box or boxes filled in the area color, every other box an outline. On slides 8 through 21 and 23. `mini-all`, every area lit in its own color, on 20, 21, and 23. Absent on slide 7, on slide 16's blank state, on slide 19 build 3, on slide 22, and on the header state of each "When you are the user" slide, where it appears with the shrink. Swapped by placing a different PNG at the same numeric position on each slide, never Morphed: Morph is a slide transition and would cross-fade the title and kicker on every area boundary. Same x and y on every slide, entered in the Format pane, so it does not jump.
...
**Gutter numeral.** The numeral of a numbered list set as a Consolas secondary run, then a tab to a fixed stop, so the numbers form a column like a line-number gutter. Not PowerPoint autonumbering, which has no plain numeral format. At the list size on 4 and 5, tab stop 40. At 44 on 17 and 23, where the slide files say the number is large, tab stop 72. Slide 22 carries no numerals.
...
- Build the deck in the order that pays off first: the slide master with its defaults, the six "When you are the owner" slides, the six "When you are the user" slides with their headers, then 1, 5, 20, and 22, then the rest. Slides 3, 6, 24, and 25 last.
...
## 11. Essential-corrections layout revision, September 15, 2026

This section supersedes conflicting row budgets above. The palette, six-column grid, type scale, source keys, and 33 physical slides stay fixed.
